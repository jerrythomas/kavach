...
async function loginSupabase(page: Page, email: string, password: string) {
	...
	// Role is set directly from known test user email — no token-claim parsing needed
	// because handleRouteProtection reads the session cookie as-is (no adapter call).
	// GoTrue's token role is always 'authenticated', never the real admin/user role.
	const role = email === 'admin@test.com' ? 'admin' : 'user'

	await setCookie(page, {
		access_token: token.access_token,
		refresh_token: token.refresh_token,
		user: { id: token.user.id, email: token.user.email, role }
	})
}
