...
<div class="bg-surface-z0 flex h-full flex-col">
	{#if screen === 'login'}
		<!-- Login: clean panel centered on surface background -->
			...
			<div class="w-full max-w-sm">
				<div class="border-surface-z3 bg-surface-z0 rounded-2xl border p-8 shadow-xl">
					...
					<div class="mb-6 text-center">
						...
						>
							<span class="{platform?.icon ?? 'i-app-shield'} text-primary h-6 w-6"></span>
						</div>
						<h1 class="text-surface-z9 text-2xl font-black">FitTrack</h1>
						<p class="text-surface-z5 mt-1 text-sm">
							Kavach + {platform?.name ?? 'Auth'} demo
						</p>
					</div>
					...
					<div class="flex flex-col gap-2">
						{#each ATHLETES as athlete (athlete.id)}
							...
							>
								...
								>
								<div
									class="bg-surface-z2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold text-cyan-600"
								>
									{athlete.name[0]}
								</div>
								<div class="min-w-0">
						...
						{#each COACHES as c (c.id)}
							...
							>
								...
								>
								<div
									class="bg-surface-z2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold text-amber-600"
								>
									{c.name[0]}
								</div>
								<div class="min-w-0">
									<p class="text-surface-z9 text-sm font-medium">{c.name}</p>
									<p class="text-surface-z5 text-xs">{c.specialty}</p>
								</div>
							</button>
						{/each}
						...
						>
							...
							>
							<div
								class="bg-surface-z2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold text-red-600"
							>
								A
							</div>
							<div class="min-w-0">
					...
					<!-- Real demo link for live platforms -->
					{#if platform?.live && platform?.url}
						<div class="border-surface-z2 mt-5 border-t pt-4">
							<a
								href={platform.url}
								target="_blank"
								rel="noopener noreferrer"
								class="text-primary hover:text-primary/80 block text-center text-xs transition-colors"
							>
								Open real {platform.name} demo →
							</a>
						</div>
					{/if}
				</div>
	...
	{:else}
		...
		<div class="flex h-full flex-col" style="background:#0a0a0f;color:#f0f0f5">
			...
			>
				...
				<span class="rounded-full bg-cyan-500/10 px-2 py-0.5 font-mono text-xs text-cyan-400"
					>{role}</span
				>
				<!-- Platform badge -->
				<span class="rounded-full bg-white/5 px-2 py-0.5 font-mono text-xs text-white/30">
					{platform?.name ?? 'Auth'}
				</span>
				...
				{#if platform?.live && platform?.url}
					<a
						...
						target="_blank"
						rel="noopener noreferrer"
						class="mr-2 rounded-lg bg-white/5 px-3 py-1.5 text-xs text-white/50 transition-colors hover:bg-white/10 hover:text-white/70"
					>
						Real demo →
					</a>
				{/if}
				...
				<!-- Hacker toggle -->
				<label class="mr-3 flex cursor-pointer items-center gap-2 text-xs text-white/40">
					<span>Hacker</span>
					<div
