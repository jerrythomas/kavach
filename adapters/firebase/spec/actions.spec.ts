...
describe('getActions (firebase)', async () => {
	...
	describe('get', () => {
		...
		it('should return error response on unsupported like operator', async () => {
			...
			expect(response.error).toEqual({
				message:
					'Firestore does not support the "like" operator. Use eq, neq, gt, gte, lt, lte, in, or is.'
			})
	...
	describe('patch', () => {
		...
		it('should return error response if id filter is missing', async () => {
			...
			expect(response.error).toEqual({
				message: 'patch requires filter.id eq.<docId> — Firestore needs a document reference'
			})
			expect(response.status).toBe(500)
		})
	})
	...
	describe('delete', () => {
		...
		it('should return error response if id filter is missing', async () => {
			...
			expect(response.error).toEqual({
				message: 'delete requires filter.id eq.<docId> — Firestore needs a document reference'
			})
			expect(response.status).toBe(500)
		})
	})
