...
export function getActions(db: any, functions?: any) {
			...
			for (const { column, op, value } of filters) {
				...
				if (firestoreOp) {
					if (op === 'in') {
						...
						constraints.push(
							where(column, firestoreOp as '==' | '!=' | '>' | '>=' | '<' | '<=', value ?? null)
						)
					}
				}
			}
	...
	async function patch(
		...
		try {
			...
			if (!idFilter)
				throw new Error(
					'patch requires filter.id eq.<docId> — Firestore needs a document reference'
				)
	...
	async function del(
		...
		try {
			...
			if (!idFilter)
				throw new Error(
					'delete requires filter.id eq.<docId> — Firestore needs a document reference'
				)
			const docRef = doc(db, entity, idFilter.value as string)
