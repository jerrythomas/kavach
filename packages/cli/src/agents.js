...
// node_modules/@kavach/cli/agents/ once installed. Agents are single .md files
...
const DEFAULT_AGENTS_URL = 'https://kavach.sensei-hq.com/agents'
...
async function clackSelect(catalog, message) {
	...
	const picked = await p.multiselect({
		message,
		options: catalog.map((a) => ({ value: a.name, label: a.name, hint: a.description })),
		required: false
	})
...
export function runAgentsList({ agentsDir, cwd = process.cwd() } = {}) {
	...
	console.info('Available Kavach review agents:\n')
	for (const a of catalog) {
		const mark = existsSync(join(installedRoot, `${a.name}.md`)) ? '✓ ' : '  '
		console.info(`${mark}${a.name}`)
		console.info(`     ${a.description}`)
	}
	console.info('\nInstall with: kavach agents add <name> [...]   (or --all)')
}

/**
 * Resolve which agents to install, then copy (or fetch with --remote) and report.
 * @param {string[]} names
 * @param {{ all?: boolean, force?: boolean, remote?: boolean, agentsDir?: string, cwd?: string, fetchImpl?: typeof fetch, promptImpl?: (catalog: Array<{name:string,description:string}>, message: string) => Promise<string[]> }} [opts]
 */
export async function runAgentsAdd(
	...
	if (all) {
		...
		selected = await promptImpl(catalog, 'Select agents to add')
	}
...
/**
 * CLI entry for `kavach agents` subcommands.
 * @param {'list' | 'add'} sub
