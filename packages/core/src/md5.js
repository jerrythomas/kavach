...
export function MD5Hash(message, options) {
	// Convert to byte array
	if (message.constructor == String)
		if (options && options.encoding === 'binary')
			message = charenc.bin.stringToBytes(message)
		else message = charenc.utf8.stringToBytes(message)
...
export function md5(message, options) {
	...
	var digestbytes = crypt.wordsToBytes(MD5Hash(message, options))
	return options && options.asBytes
		? digestbytes
		: options && options.asString
		? charenc.bin.bytesToString(digestbytes)
