...
describe('Internal functions', () => {
	beforeEach(() => {
		global.Response = vi.fn().mockImplementation(function (...args) {
			return args
		})
	})
	afterEach(() => {
	...
	describe('extractKeyValuePair', () => {
		it('Should extract key value pair from string', () => {
			let result = extractKeyValuePair('foo=bar')
			expect(result[0]).toEqual('foo')
			expect(result[1]).toEqual('bar')

			result = extractKeyValuePair('foo')
			expect(result[0]).toEqual('foo')
			expect(result[1]).toEqual('')

			result = extractKeyValuePair('foo|bar', '|')
			expect(result[0]).toEqual('foo')
			expect(result[1]).toEqual('bar')
		})
	})
	...
	describe('setHeaderCookies', () => {
		...
		it('Should set cookies in headers', () => {
			...
			expect(headers).toEqual({
				'Set-Cookie': [
					'data=%7B%22something%22%3A%22bar%22%7D; Max-Age=60; Path=/; HttpOnly; Secure; SameSite=Strict',
					'foo=bar; Max-Age=60; Path=/; HttpOnly; Secure; SameSite=Strict'
				]
			})
		})
	})
